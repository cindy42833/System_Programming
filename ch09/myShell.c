...
int main (int argc, char** argv) {
    ...
    while(1) {
        ...
        */
        getcwd(cwd, 256);
        pos=strspn(getenv("HOME"), cwd);    // strspan(str1, str2) return the first position of character in str1 but not in str2
        homeLen = strlen(getenv("HOME"));
        printf("cwd=%s, home=%s, pos=%d, prompt=%s", cwd, getenv("HOME"), pos, &cwd[pos]);
        if(pos>=homeLen) {  // hidden the path before home directory
            cwd[pos-1]='~';
            showPath=&cwd[pos-1];
        }
        else
        ...
        */
        ...
        if (strcmp(exeName, "cd") == 0) {
            if (strcmp(argVect[1], "~")==0)     // cd ~ change to home directory
                chdir(getenv("HOME"));
            else
                chdir(argVect[1]);
            continue;
